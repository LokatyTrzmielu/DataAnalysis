  ...
  <div class="fixed inset-0 flex items-center justify-center z-50" style="background:rgba(0,0,0,0.6)" @click.self="$emit('close')">
    <div class="card-apple-elevated w-full" style="max-width:480px">
      <div class="flex items-center justify-between mb-5">
        <h3 style="font-size:21px;font-weight:700;color:var(--app-text);letter-spacing:0.231px;line-height:1.19">Share analysis</h3>
      ...
      <div v-else-if="shares.length === 0" style="font-size:14px;color:var(--app-text-sec)">Not shared with anyone yet.</div>
      <ul v-else style="border-top:1px solid rgba(0,0,0,0.08)">
        <li v-for="s in shares" :key="s.user_id" class="flex items-center justify-between py-3" style="border-bottom:1px solid rgba(0,0,0,0.08)">
          <div>
            <span style="font-size:14px;font-weight:600;color:var(--app-text);letter-spacing:-0.224px">{{ s.name }}</span>
            <span class="ml-2" style="font-size:12px;color:var(--app-text-sec)">{{ s.email }}</span>
          </div>
          <button
