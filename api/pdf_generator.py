...
def generate_capacity_pdf(
    ...
    """Render a capacity and/or performance report.

    ``capacity_data`` may be ``None`` when the user wants a performance-only
    export. In that case the capacity section is skipped and the title is
    adjusted accordingly. At least one of capacity/performance must be present.
    """
    if not capacity_data and not performance_data:
        raise ValueError("Either capacity_data or performance_data must be provided.")
    ...
    # ── Header ──────────────────────────────────────────────────────────────
    if capacity_data and performance_data:
        title_text = 'Capacity &amp; Performance Report'
    elif capacity_data:
        title_text = 'Capacity Report'
    else:
        title_text = 'Performance Report'

    if user is not None:
        u_name = getattr(user, "name", None) or ""
        u_email = getattr(user, "email", None) or ""
        generated_by = f"{u_name} <{u_email}>" if u_name and u_email else (u_name or u_email or "—")
    else:
        generated_by = "—"
    ...
    story.append(Paragraph(
        f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")}  |  Generated by: {generated_by}',
        small_style,
    ))
    ...
    if performance_data:
        if capacity_data:
            ...
            # No capacity section — avoid an empty page break at the top.
...
def generate_container_order_pdf(plan, params, run, user=None) -> bytes:
    ...
    # Wrappable footprint label — the catalog labels go up to ~22 chars
    # ("2 cells (split length)"), which would overflow a fixed cell. Rendering
    # the label as a Paragraph lets ReportLab wrap to a second line cleanly.
    fp_style = ParagraphStyle(
        'fp', parent=styles['Normal'], fontSize=9,
    ...
    for s in plan.summaries:
        ...
        cap_data.append([
            s.code,
            Paragraph(s.footprint_label, fp_style),
            cell,
    ...
    # Widths sized to the worst-case content at 9 pt Helvetica plus 4 pt of
    # horizontal padding each side: variant codes up to "1/12_3x4-288" (12 ch),
    # cell dims up to "617×408×128" (11 ch), numeric headers (Locations,
    # Locs/bin, Avg fill) all 8–9 char wide. Footprint stays at 3.4 cm and
    # wraps inside its Paragraph if a label still overflows.
    t1 = Table(cap_data, colWidths=[2.5 * cm, 3.4 * cm, 2.4 * cm, 1.8 * cm,
                                     1.2 * cm, 2.1 * cm, 1.3 * cm, 1.9 * cm])
    ...
    # Wrap heading + table in KeepTogether so the procurement breakdown never
    # splits across pages — ReportLab will insert a page break before it if
    # the current page can't hold the whole element.
